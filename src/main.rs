...
struct Context<'a, 'b>{ msg: Option<(&'a str, &'b str)>, todos: Vec<Todo> }

impl<'a, 'b> Context<'a, 'b> {
    pub fn err(conn: &DbConn, msg: &'a str) -> Context<'static, 'a> {
        Context{msg: Some(("error", msg)), todos: Todo::all(conn)}
    }

    pub fn raw(conn: &DbConn, msg: Option<(&'a str, &'b str)>) -> Context<'a, 'b> {
        Context{msg: msg, todos: Todo::all(conn)}
    }
}

#[post("/", data = "<task_form>")]
fn new(task_form: Form<Task>, conn: DbConn) -> Flash<Redirect> {
    let task = task_form.into_inner();
    if task.description.is_empty() {
        Flash::error(Redirect::to("/"), "Description cannot be empty.")
    } else if Todo::insert(task, &conn) {
...
fn toggle(id: i32, conn: DbConn) -> Result<Redirect, Template> {
    if Todo::toggle_with_id(id, &conn) {
        Ok(Redirect::to("/"))
    } else {
...
fn delete(id: i32, conn: DbConn) -> Result<Flash<Redirect>, Template> {
    if Todo::delete_with_id(id, &conn) {
